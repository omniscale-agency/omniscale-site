...
export default function CalendarPage() {
  ...
  useEffect(() => {
    if (!client) return;
    const refresh = async () => setExtras(await fetchEvents(client.slug));
    refresh();
  ...
  const allEvents = [...extras, ...client.upcomingEvents].sort((a, b) => new Date(a.startsAt).getTime() - new Date(b.startsAt).getTime());
    ...
    <main className="p-6 md:p-10 lg:p-12 max-w-5xl mx-auto">
      <div className="mb-10 flex items-end justify-between">
        <div>
          ...
          <p className="text-white/60 mt-2">Synchronisé avec Google Calendar (à brancher)</p>
        </div>
        <a href="#" className="inline-flex items-center gap-2 text-sm text-white/70 hover:text-lilac border border-white/15 hover:border-lilac/40 px-4 py-2 rounded-lg transition-colors">
      ...
      <div className="space-y-3">
        {allEvents.map((e) => {
          ...
          return (
            <div key={e.id} className="rounded-2xl border border-white/10 bg-white/[0.02] p-5 flex items-center gap-5 hover:border-lilac/30 transition-colors">
              ...
              <button className="bg-lilac text-ink font-medium px-4 py-2 rounded-lg text-sm hover:bg-white transition-colors">Rejoindre</button>
            </div>
          );
        })}
      </div>
